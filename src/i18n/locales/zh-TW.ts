...
const translations = {
  ...
  remoteSyslog: {
    title: '遠端日誌',
    sectionTitle: '遠端日誌目的地',
    description: '設定遠端 syslog 伺服器以接收裝置日誌。每種日誌類型可轉發至不同目的地。',
    ...
    errors: {
      typeRequired: '必須選擇日誌類型',
      addressRequired: '必須輸入伺服器位址',
      invalidIp: 'IP 位址格式錯誤',
      invalidPort: '埠號必須介於 1 到 65535 之間',
      typeDuplicate: '此日誌類型已設定遠端目的地',
      destinationDuplicate: '目的地 {destination} 已被 {type} 使用。請使用不同的 IP:埠號組合以避免 syslog-ng persist-name 衝突。',
      loadFailed: '載入設定失敗',
      saveFailed: '儲存設定失敗',
      deleteFailed: '刪除設定失敗'
    },
    confirmDelete: '確定要刪除 {type} 的遠端目的地嗎？',
    updateSuccess: '設定更新成功',
    deleteSuccess: '{type} 遠端目的地刪除成功'
  },
  operationMode: {
    title: '操作模式管理',
    mode: '模式',
